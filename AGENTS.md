...
<!-- ── Fork-local sections (Craesol/career-ops) ── the next four sections are
     fork governance, not upstream content. They moved here from CLAUDE.md so
     every CLI wrapper (CLAUDE.md, CODEX.md, OPENCODE.md, …) loads them, and
     CLAUDE.md stays the thin @AGENTS.md wrapper the test suite requires. If an
     upstream update ever overwrites this file, restore them from git history. -->
...
## Fork test-suite deviations (accepted, 2026-09-06)

`node test-all.mjs` on this fork ends **~15 red / ~20 warnings by design** — do
not "fix" these, and treat any OTHER failure as a real regression:

1. **Tracked user-layer data (6 tests):** `portals.yml`, `data/pipeline.md`,
   `data/scan-history.tsv`, `data/portal-health.tsv`,
   `interview-prep/story-bank.md` are deliberately git-tracked so state syncs
   between this fork's machines via origin pushes. Upstream wants them
   gitignored; the fork does not.
2. **Fork web app vs upstream web contracts (8 tests):** `web/` here is the
   fork's own app. Upstream's parity checks (#2185 write-scope modules, #2666
   normalize-text-key mirror, profile-keywords, followups/cadence route,
   tracker-columns web parity) reference upstream web files this app does not
   have. Porting the #2666 shared-key mirror is tracked as follow-up work.

Fork-local automation files are registered in `config/local-paths.txt`
(gitignored — travels between machines in the personal zip); never register
them by editing `USER_PATHS` in `update-system.mjs`, which every
`update-system.mjs apply` overwrites (that is how the 27a6ec0 registration
was silently lost in the v1.32.0 update).
