...
<!-- ── Fork-local sections (Craesol/career-ops) ── the next three sections are
...
**Req/posting ID in notes disambiguates same-title postings (#1524, #2009):** when a company posts two genuinely different requisitions whose titles fuzzy-match (e.g. a leveled variant and its bare title, or two sibling team roles), put the req/job/posting ID in the **notes** column on both rows. `merge-tracker.mjs` and `dedup-tracker.mjs` read it (`REQ_NUMBER_RE`, shared via `tracker-parse.mjs`) and treat rows carrying *different* recognizable IDs as distinct openings, overriding fuzzy/exact title matching. Recognized forms are a `job id` / `posting id` / `requisition` / `req` / `jr` / `job` / `posting` / `ref` / `r_` label followed by an alphanumeric ID containing at least one digit — e.g. `req JR-10423`, `job id 88214`, `ref R_2291`. Prefer this whenever the JD exposes an ID; it is the only signal that survives near-identical titles.
